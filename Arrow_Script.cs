...
public class Arrow_Script : MonoBehaviour {
   ...
   public GameObject marker4;
    public static float shift = 0f;
    //public static float shift = 3f;
   public float k = 5;
   private float current;
   private Vector3 scale;
   public float scalefactorGrow;
   ...
   MixedRealityPose virtualHand;
    ...
    void Start() {
      ...
      initialScale = palmObject.transform.localScale.x;
    }
   ...
   void ShiftFunction(float extend, float shrink) {
      ...
      if (distance > extend) {
         ...
         if (shift > 0f) // Modify this for how far you want the object to be
            shift -= 0.01f;
      }
   }
   ...
   void PositionControl(float extend, float shrink) {
     ...
     // if (distance > extend - (extend - shrink) / 2) {
            ...
            //shift = k * Mathf.Pow((distance-(extend/2)), 2);
        //}
   }
   ...
   void Scaling() {
      if (scaleControl) {
            ...
            dCurr1 = Vector3.Distance(palmObject.transform.position, Camera.main.transform.position);

            scalefactorGrow = initialScale * (dCurr1 / extend);

            if ((dCurr1 > shrink) && (scalefactorGrow > 0.5f)) {

                Debug.Log("IN SCALE CONTROL ");

                //Modifying Both hands
